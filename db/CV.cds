...
using {
    CD_DISTRIBUTION_CHANNEL as CD_DISTRIBUTION_CHANNEL,
...
context CV {
    ...
    view ConfigurationItemView as
        select from PR.ConfigurationItem {
                @title: 'Menu Id'
            key ConfigurationItem.MENUID          as MenuId,
    ...
    view PricingRuleView as
        ...
        left outer join PR.RuleTypeText
            on PricingRule.RULETYPEID = RuleTypeText.RULETYPEID
        {
                @title: 'Rule ID'
            key PricingRule.RULEID        as RuleId,
                ...
                @title: 'Priority'
                cast(
                    PricingRule.PRIORITY as Integer
                )                         as Priority
        }
        where
    ...
    view AccessSequenceView as
        ...
        left outer join PR.PricingRule
            on AccessSequence.RULEID = PricingRule.RULEID
        left outer join PR.RuleText
            on AccessSequence.RULEID = RuleText.RULEID
        left outer join PR.EventTypeText
            on AccessSequence.EVENTTYPEID = EventTypeText.EVENTTYPEID
        left outer join PR.DimensionText
            on AccessSequence.DIMENSIONID = DimensionText.DIMENSIONID
        {
                @title: 'Access Sequence Uuid'
            key AccessSequence.ACCESSSEQUENCEUUID as AccessSequenceUuid,
                ...
                @title: 'Priority'
                cast(
                    AccessSequence.PRIORITY as Integer
                )                                 as Priority
        };
    ...
    */
    ...
    view MessageTypeMaintenanceView as
        ...
        left outer join PR.PricingRule
            on MessageTypeMaintenance.RULEID = PricingRule.RULEID
        left outer join PR.RuleText
            on MessageTypeMaintenance.RULEID = RuleText.RULEID
        left outer join PR.EventTypeText
            on MessageTypeMaintenance.EVENTTYPEID = EventTypeText.EVENTTYPEID
    ...
    */
    ...
    view RuleDefinitionView as
        ...
        left outer join PR.RuleText
            on RuleDefinition.RULEID = RuleText.RULEID
        {
                @title: 'Rule Id'
            key RuleDefinition.RULEID     as RuleId,
    ...
    */
    ...
    view SystemMessageView as
        select from PR.PricingRule
        left outer join CA.MessageClassText
            on PricingRule.MESSAGENUMBER = MessageClassText.MESSAGENUMBER
        left outer join PR.RuleText
            on PricingRule.RULEID = RuleText.RULEID
        {
                @title: 'Rule Id'
    ...
    */
    ...
    view EventRuleConditionView as
        select from PR.EventRuleCondition {
            ...
            key EventRuleCondition.EVENTRULECONDITIONUUID as EventRuleConditionUuid,

                @title: 'Rule Condition Uuid'
            key EventRuleCondition.RULECONDITIONUUID      as RuleConditionUuid,
    ...
    */
    ...
    view EventRuleDimensionView as
        ...
        {
                @title: 'Event Rule Condition Uuid'
            key EventRuleDimension.RULECONDITIONUUID as RuleConditionUuid,

                @title: 'Dimension Id'
            key EventRuleDimension.DIMENSIONID       as DimensionId,

                @title: 'Dimension Description'
                DimensionText.DESCRIPTION            as Dimension,

                @title: 'Dimension Value'
            key EventRuleDimension.DIMENSIONVALUE    as DimensionValue
        };
    ...
    view RuleMessageView as
        ...
        left outer join PR.RuleText
            on PricingRule.RULEID = RuleText.RULEID
        inner join PR.MessageTypeMaintenance
            on PricingRule.RULEID = MessageTypeMaintenance.RULEID
        left outer join PR.EventTypeText
            on MessageTypeMaintenance.EVENTTYPEID = EventTypeText.EVENTTYPEID
        inner join CA.MessageType
            on MessageTypeMaintenance.MESSAGETYPEID = MessageType.MESSAGETYPEID
        left outer join CA.MessageClassText
            on PricingRule.MESSAGENUMBER = MessageClassText.MESSAGENUMBER
        {
    ...
    */
    ...
    view RuleScreenConfigurationView as
        select from PR.RuleScreenConfiguration {
                @title: 'Rule Id'
            key RuleScreenConfiguration.RULEID                  as RuleId,
